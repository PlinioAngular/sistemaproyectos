				...
				<form id="add_comprobante" name="add_comprobante" accept-charset="utf-8" enctype="multipart/form-data" method="post">
					...
					<div class="row">
						...
						<div class="form-group">
								<label for="">Nombre Comprobante</label><input autocomplete="off" class="form-control form-control-sm" placeholder="Nombre:" type="text" name="comprobante" id="comprobante">
							</div>
						</div>					
					</div>													
...
$(document).ready(function() {
		$("form[name='add_comprobante']").submit(function(e) {
			var formData = new FormData($(this)[0]);
			$.ajax({
				
				url: "<?php echo base_url('comprobante/comprobante_add'); ?>",
				type: "POST",
				data: formData,
				async: true,
				beforeSend: function(){
